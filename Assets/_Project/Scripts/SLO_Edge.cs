...
public class SLO_Edge : MonoBehaviour
{
	...
	public float Diameter
	{
		set
		{
			...
			if( MeshEdgeSolver.Instance.m_JointType == SLO_Join.JointType.Outer )
				m_TubeMesh.OutsideRadius = ( m_Diameter / 2f ) * .001f;
			else if( MeshEdgeSolver.Instance.m_JointType == SLO_Join.JointType.Inner )
			{
				m_TubeMesh.OutsideRadius = ( (m_Diameter / 2f) + (MeshEdgeSolver.Instance.WallThickness*2)  ) * .001f;
	...
	public void Init(Vector3 vertexPos0, Vector3 vertexPos1 )
	{
