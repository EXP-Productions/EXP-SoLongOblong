...
public class MeshEdgeSolver : MonoBehaviour
{
	...
	void LayFacesFlat()
	{
		...
		for (int i = 0; i < m_Faces.Count; i++) 
		{
			...
			//newTriMesh.transform.position = midPoint;
			//newTriMesh.transform.LookAt( midPoint + m_Faces[i].Normal );
			...
			//newTriMesh.UpdateTri( _Faces[i].Join0.transform.position,  m_Faces[i].Join0.transform.position,  m_Faces[i].Join0.transform.position, .1f);
